...
(KTD-4a). A live `grok inspect --json` probe confirmed that `~/.grok/AGENTS.md`
...
Native subagent surfaces (U11) project Soma onto Grok's own primitives, read by
...
in the chain (KTD-3). A `PreToolUse` hook's stdout-deny IS honored (the U1 gate
...
decoding would otherwise corrupt non-ASCII bytes (KTD-2).

`installSomaForGrok` refuses runtimes below `0.2.38`, read from
`~/.grok/version.json` with no live `grok` exec (KTD-9); a missing manifest is
