...
results were institutional knowledge held only in `soma-notes/grok-adapter/`,
not in the durable rule-record.
...
- **Context auto-load (KTD-4a).** `grok inspect --json` confirmed that
...
- **Fail-closed deny (U1).** A `PreToolUse` hook that writes a deny to stdout and
  ...
  error rather than trusting a single gate (KTD-3).
- **Passive-hook stdout is ignored (2026-06-10-004).** Only `PreToolUse` reads
  ...
  on passive stdout.
- **Per-session lifecycle cardinality (U1).** `SessionStart`/`SessionEnd` fire
...
- **Windows hook execution (KTD-2).** Hooks are bare-exec'd against the resolved
...
- Adapter behavior is documented in `docs/substrate-adapters.md` (Grok section);
  the probe transcripts live in `soma-notes/grok-adapter/2026-06-10-00{2,3,4}-*`.
- No test launches a live `grok`; hook tests spawn via `node` with `HOME`/
  `USERPROFILE` pinned (KTD-9), so CI stays deterministic.
...
**Discussion:** U1 gate pass and 0.2.39 re-probes, 2026-06-10.
