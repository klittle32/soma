/**
 * Post-install smoke of the frozen grok hook command (plan 006, R5).
 *
 * Grok's hook platform is fail-open (KTD-3): a hook command that
 * cannot launch is not an error — the tool call is silently allowed,
 * disabling Soma's only Windows policy gate. R3 validates the bun
